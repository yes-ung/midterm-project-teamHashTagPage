...
<body>
	...
	<!-- 나중에 쓸 코드(출판사DB 완성시 사용 -->
	<%-- ${Author.authorName} --%>
	<!-- 나중에 쓸 코드(작가DB 완성시 사용 -->
	<div class="page">
		...
		<main class="webnovel-series-detail mt4">
			<div class="series-flex-row">
				...
				<div class="webnovel-series-info">
					...
					<p class="mb1">
						✍ <a
							href="<c:url value='/author.do'><c:param name='authorId' value="${series.authorId}" /></c:url>"
							class="atext">${series.authorId}</a> | 🏷️ ${series.typeId}
					</p>
					<p class="mb1">
						📖 총 ${fn:length(episode)}화 | 📚 <a
							href="<c:url value='/publisher.do'><c:param name='publisherId' value="${series.publisherId}" /></c:url>"
							class="atext">${series.publisherId}</a>
					</p>
					<p class="mb1">⭐ 평점 4.9 (13,753명)</p>
					<div class="button-group">
						<a
							href="<c:url value='/hashtagpage/webnovel/detail.do'><c:param name='seriesId' value="${series.seriesId}" />
              <c:param name='episodeId' value="1" /></c:url>"
							class="a-rounded-al mr2">첫화보기</a>
			...
			<div class="book-description mt4">
				...
				<c:if test="${fn:length(series.seriesDescription) > 50}">
					<p class="description">
						...
						<c:out
							value="${fn:substring(series.seriesDescription, 50, fn:length(series.seriesDescription))}" />
			...
			<section class="webnovel-episode-list mt5">
				...
				<div class="array-btn">
					...
					<select class="array-select">
						<option value="first">1화부터</option>
				...
				<ul class="episodeContainer">
					...
					<c:forEach var="data" items="${episode}">
						...
						<li class="episode-item mb1"><a
							href="<c:url value='/hashtagpage/webnovel/detail.do'><c:param name='seriesId' value="${data.seriesId}" />
              <c:param name='episodeId' value="${data.episodeId}" /></c:url>"
							class="episode-link">
								<div class="thumb">
